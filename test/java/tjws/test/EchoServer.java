 ...
 *                 This will be accessed via
 *                 ws://localhost:8080/EchoChamber/echo
 *                 Where "localhost" is the address of the host,
 *                 "EchoChamber" is the name of the package
 *                 and "echo" is the address to access this class from the
 *                 server
 */
...
public class EchoServer {
	 ...
	 *         In the method onOpen, we'll let the user know that the handshake
	 *         was
	 *         successful.
	 */
	@OnOpen
	public void onOpen(Session session, EndpointConfig config) {
		...
		boolean htp_sess = config.getUserProperties().containsKey(HttpSession.class.getName());
		try {
